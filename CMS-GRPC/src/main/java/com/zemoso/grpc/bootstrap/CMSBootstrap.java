...
public class CMSBootstrap implements ApplicationListener<ContextRefreshedEvent> {
	...
	private void init() {
		...
		Comments savedComment = commentsService.addComments(comment);

		// Comments 2 
		
		comment = new Comments();
		comment.setContent("Some comment for blog 2");
		comment.setCommentsdate(LocalDate.of(2020, 03, 18).format(DateTimeFormatter.ofLocalizedDate(FormatStyle.FULL)));
		comment.setRefblog(savedBlog2);
		comment.setRefuser(savedUser2);
		Comments savedComment2 = commentsService.addComments(comment);
		System.out.println("Comment also got saved!");
