...
import com.zemoso.grpc.CmsBlogGrpc.CmsBlogImplBase;
import com.zemoso.grpc.bootstrap.BeanUtil;
...
public class CmsBlogServiceImpl extends CmsBlogImplBase {
	...
	public void login(Usercredential request, StreamObserver<Grpcsocialuser> responseObserver) {
		System.out.println("Login request came for user: " + request.getUsername());
	...
	public void post(Logincms request, StreamObserver<Bloglist> responseObserver) {
		System.out.println("Service request came from CmsBlog for post with id: " + request.getResponsecode());
	...
	public void create(Blog request, StreamObserver<Blog> responseObserver) {
		System.out.println("\n\nBlog create request...");
		BlogService blogService = BeanUtil.getBean(BlogService.class);
		SocialusersService useService = BeanUtil.getBean(SocialusersService.class);
		Post post = new Post();

		post.setContent(request.getContent());
		post.setCreationdate(LocalDate.now().format(DateTimeFormatter.ofLocalizedDate(FormatStyle.FULL)));

		Socialuser user = useService.getUserByEmail(request.getRefuser());
	...
	public void delete(DeletePost request, StreamObserver<Blog> responseObserver) {
		System.out.println("Delete Blog request came from CmsBlog with blog id: " + request.getBlogid());
		BlogService blogService = BeanUtil.getBean(BlogService.class);
	...
	public void postcomment(Comment request, StreamObserver<Blog> responseObserver) {
		System.out.println("Comment request came from CmsBlog for blogId: " + request.getBlogid());
